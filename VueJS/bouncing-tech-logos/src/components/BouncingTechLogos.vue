  ...
  setup(props) {
    ...
    const loadAvailableLogos = async () => {
      try {
        ...
        console.warn('⚠️ Fallback vers liste statique')
    ...
    const onImageError = (event, logo) => {
      console.warn(`⚠️ Erreur chargement: ${logo.name}`)
    ...
    // 🖱️ CLIC SUR LOGO = OUVERTURE PAGE OFFICIELLE
    const onLogoClick = (logo) => {
      ...
      if (url) {
        ...
        console.warn(`⚠️ URL non trouvée pour ${logo.name}`)
      }
    }
    ...
    const updateContainerBounds = () => {
      ...
      try {
        ...
        console.warn('⚠️ Erreur mesure DOM, utilisation valeurs par défaut')
      }
