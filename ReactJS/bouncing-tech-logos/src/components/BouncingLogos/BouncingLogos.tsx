...
const BouncingLogos: React.FC<BouncingLogosProps> = ({
  ...
  // Tous les logos tech disponibles (16 au total)
  ...
  const createLogo = useCallback((logoName: string, index: number): Logo => {
    ...
    return {
      ...
      x: x,
      y: y,
      vx: (Math.random() - 0.5) * 1.0, // Vitesse de départ augmentée (1.15 → 1.5)
      vy: (Math.random() - 0.5) * 1.0, // Vitesse de départ augmentée (1.15 → 1.5)
      rotation: 0,
  ...
  const updateLogos = useCallback(() => {
    ...
    setLogos(prev => {
      ...
      for (let i = 0; i < newLogos.length; i++) {
        for (let j = i + 1; j < newLogos.length; j++) {
          ...
          if (distance < minDistance) {
            ...
            const maxSpeed = 2.50; // +15% de vitesse max (3.0 → 3.45)
      ...
      newLogos.forEach(logo => {
        const wallBoost = 0.80; // +25% d'énergie au lieu de -15% (inverse de 0.85)
        const maxWallSpeed = 3.0; // PLAFOND AUGMENTÉ pour rebonds plus énergiques ! (4.0 → 5.0)
